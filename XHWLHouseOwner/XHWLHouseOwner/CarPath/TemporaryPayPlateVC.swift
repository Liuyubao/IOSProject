...
class TemporaryPayPlateVC: UIViewController {
    ...
    @IBOutlet weak var temporaryPayView: UIView!
    ...
    @IBAction func conformBtnClicked(_ sender: UIButton) {
        ...
        carPayInfoVC.carPlate.text = getWholePlateNumber()
    ...
    fileprivate func initInputCarNumebrView() {
        ...
        // 创建 车牌号显示的View
        inputCarNumebrButton.frame = CGRect(x: 50, y: 90, width:temporaryPayView.frame.width-100, height: 60)
        
        temporaryPayView.addSubview(inputCarNumebrButton)
        
        // 加载里面内容
        inputCarNumebrButton.inputCarNumebrViewLoadContent()
        
        // 添加事件
        inputCarNumebrButton.addTarget(self, action: #selector(TemporaryPayPlateVC.clickInputCarNumebrButton(_:)), for: .touchDown)
...
// MARK:
// MARK: 自定义键盘的 代理方法
extension TemporaryPayPlateVC: XNKeyboardViewDelegate {
    
    // MARK:
    // MARK: 代理方法 监听点击 赋值
    func getWholePlateNumber() -> String{
        var wholePlateNumber = ""
        for label in inputCarNumebrButton.labelArray{
            if label.text != nil{
